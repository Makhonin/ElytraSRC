...
bool AP_AHRS_NavEKF::healthy(void) const
{
    ...
    if (_ekf_use != EKF_DO_NOT_USE) {
		//hal.console->println_P(PSTR("Using Calman-Ironman"));
        bool ret = ekf_started && EKF.healthy();
        if (!ret) {
			hal.console->println_P(PSTR("Ironman said you are gay"));
            return false;
        }
        if ((_vehicle_class == AHRS_VEHICLE_FIXED_WING ||
             _vehicle_class == AHRS_VEHICLE_GROUND) &&
            ...
            hal.console->println_P(PSTR("Strange."));
            return false;
        }
         //hal.console->println_P(PSTR("Ok."));
        return true;
    }
    hal.console->println_P(PSTR("Using DCM"));
