...
void AP_Baro::calibrate()
{
    ...
    hal.console->println("Setting params...");
    for (uint8_t i=0; i<_num_sensors; i++) {
        if (count[i] == 0) {
    ...
    for (uint8_t i=0; i<_num_sensors; i++) {
        if (sensors[i].calibrated) {
			hal.console->println("End of calibration!");
            return;
        }
    }
...
hal.console->println("Baro PX4");
    drivers[0] = new AP_Baro_PX4(*this);
    _num_drivers = 1;
#elif HAL_BARO_DEFAULT == HAL_BARO_HIL
hal.console->println("Baro HIL2");
    drivers[0] = new AP_Baro_HIL(*this);
    _num_drivers = 1;
#elif HAL_BARO_DEFAULT == HAL_BARO_BMP085
hal.console->println("Baro BMP");
    {
        drivers[0] = new AP_Baro_BMP085(*this);
        _num_drivers = 1;
    }
#elif HAL_BARO_DEFAULT == HAL_BARO_MS5611 && HAL_BARO_MS5611_I2C_BUS == 0
hal.console->println("Baro MS5611");
    {
       /* drivers[0] = new AP_Baro_MS5611(*this, new AP_SerialBus_I2C(hal.i2c, HAL_BARO_MS5611_I2C_ADDR), false);
        _num_drivers = 1;*/
        hal.console->println("Heh, Baqqro LPS331 for Elytra!");
        drivers[0] = new AP_Baro_LPS331(*this, new AP_SerialBus_I2C(hal.i2c, HAL_BARO_LPS331_I2C_ADDR), false);
         hal.console->println("Baro LPS331 for Elytra3!");
        _num_drivers = 1;
         hal.console->println("Baro LPS331 for Elytra3!");
    }
#elif HAL_BARO_DEFAULT == HAL_BARO_MS5611_SPI
    {
		hal.console->println("Baro MS5611_SPI");
    ...
    {
		hal.console->println("Baro MS5607");
        drivers[0] = new AP_Baro_MS5607(*this, new AP_SerialBus_I2C(hal.i2c1, HAL_BARO_MS5607_I2C_ADDR), true);
        _num_drivers = 1;
    }
#elif HAL_BARO_DEFAULT == HAL_BARO_LPS331
    {
		hal.console->println("Baro LPS331 for Elytra!");
        drivers[0] = new AP_Baro_LPS331(*this, new AP_SerialBus_I2C(hal.i2c, HAL_BARO_LPS331_I2C_ADDR), false);
        hal.console->println("Baro LPS331 for Elytra1!");
        _num_drivers = 1;
        hal.console->println("Baro LPS331 for Elytra3!");
    }    
#endif    
hal.console->println("Baro LPS331 for Elytra311!");
    if (_num_drivers == 0 || _num_sensors == 0 || drivers[0] == NULL) {
		hal.console->println("Baro LPS331 for Elytra3111!");
        hal.scheduler->panic(PSTR("Baro: unable to initialise driver"));
    }
    hal.console->println("Baro LPS331 for Elytra4!");
}
...
void AP_Baro::update(void)
{
    if (!_hil_mode) {
        for (uint8_t i=0; i<_num_drivers; i++) {
			//hal.console->println("LPS331:update");
            drivers[i]->update();
        }
    }
    ...
    for (uint8_t i=0; i<_num_sensors; i++) {
        ...
        {
			//hal.console->println("Detected not healthy!");
		}
    }
    ...
    for (uint8_t i=0; i<_num_sensors; i++) {
        if (sensors[i].healthy) {
            ...
            if (is_zero(sensors[i].ground_pressure)) {
               ...
               // hal.console->println("Ground pressure is zero!");
            }
            sensors[i].altitude = true;//get_altitude_difference(sensors[i].ground_pressure, sensors[i].pressure);
            // sanity check altitude
            sensors[i].alt_ok = !(isnan(sensors[i].altitude) || isinf(sensors[i].altitude));
            if (!sensors[i].altitude)
			{
				//hal.console->println("Detected not correct alt!");
			}
			if (!sensors[i].alt_ok)
			{
				//hal.console->println("Detected not correct alt_ok!");
			}
			else
			{
				//hal.console->println("Detected alt_ok!");
			}
        }
    }
