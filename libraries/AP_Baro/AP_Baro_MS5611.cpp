...
int16_t AP_SerialBus_I2C::read_16bits_2(uint8_t reg)
{
    uint8_t buf[2];
    if (_i2c->readRegisters(_addr, reg, sizeof(buf), buf) == 0) {
...
uint32_t AP_SerialBus_I2C::read_24bits_2(uint8_t reg)
{
    uint8_t buf[3];
    if (_i2c->readRegisters(_addr, reg, sizeof(buf), buf) == 0) {
...
void AP_SerialBus_I2C::writeReg(uint8_t reg, uint8_t value)
{ 
    if (_i2c->writeRegister(_addr, reg,value)!=0)
	{
		hal.console->println("I2C error.");
	}
}
...
// reads register
uint8_t AP_SerialBus_I2C::readReg(uint8_t reg)
{
    uint8_t i2cres=0;
    if (_i2c->readRegister(_addr, reg,&i2cres)!=0)
	{
		hal.console->println("I2C error.");
	}
    return i2cres;
