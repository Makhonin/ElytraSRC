...
*/
		...
		*/
				...
				_data[_data_idx].gyro_filtered = gyro_f;
                _have_gyro_sample = true;
...
void AP_InertialSensor_L3G4200D::async_accumulate(void)
{
	...
	if (reg==0)
	{
		...
		if (fifo_status & L3G4200D_REG_FIFO_SRC_OVERRUN) {
			...
			// FIFO is partly full
			num_s_available = fifo_status & L3G4200D_REG_FIFO_SRC_ENTRIES_MASK;
		}

		if (num_s_available > 0) 
		{
			// read all the entries in one go, using AUTO_INCREMENT. This saves a lot of time on I2C setup
			int16_t bufferG[num_s_available][3];
			if (hal.i2c->readRegisters(L3G4200D_I2C_ADDRESS, L3G4200D_REG_XL | L3G4200D_REG_AUTO_INCREMENT,
			sizeof(bufferG), (uint8_t *)&bufferG[0][0]) == 0) 
			{
				for (uint8_t i=0; i<num_s_available; i++) {
                gyro_f = _gyro_filter.apply(Vector3f(bufferG[i][1], bufferG[i][0], -bufferG[i][2]));
	...
	{
		for (uint8_t i=0; i<num_s_available; i++) {
                _gyro_filter.apply(Vector3f(buffer[i][0], -buffer[i][1], -buffer[i][2]));
            }
        reg=LIS331DLH_I2C_OUT_X;
		return;
	}*/
