    ...
    for (int16_t j = 0; j <= 30*4 && num_converged < num_gyros; j++) {
        ...
        for (i=0; i<50; i++) {
            ...
            for (uint8_t k=0; k<num_gyros; k++) {
              ...
              //  hal.console->printf_P(PSTR("Gyro sum is: %f"),gyro_sum[k].length());
            }
            hal.scheduler->delay(5);
        }
		hal.console->print_P(PSTR("Data got!"));
        ...
        if (accel_diff.length() > 0.2f) {
            ...
            //hal.console->printf_P("accel %f %f %f \n",accel_start.x,accel_start.y,accel_start.z);
            hal.console->println("Noo,nooo....");
            continue;
        }
        else
        {
        ...
        for (uint8_t k=0; k<num_gyros; k++) {
            ...
            //hal.console->printf_P("gyro %f %f %f \n",gyro_avg[k].x,gyro_avg[k].y,gyro_avg[k].z);
        ...
        for (uint8_t k=0; k<num_gyros; k++) {
            if (j == 0) {
                ...
                hal.console->printf_P(PSTR("Gyro diff1 is: %f"),gyro_diff[k].length());
            } else if (diff_norm[k] < best_diff[k]) {
				
				hal.console->printf_P(PSTR("Gyro diff2 is: %f"),gyro_diff[k].length());
                best_diff[k] = diff_norm[k];
...
void AP_InertialSensor::async_update(void)
{
	for (uint8_t i=0; i<_backend_count; i++) 
	{
            _backends[i]->async_accumulate();
     }
}
/*
    ...
    if (!_hil_mode) {
        ...
        for (uint8_t i=0; i<INS_MAX_INSTANCES; i++) {
            if (_gyro_healthy[i] && _gyro_error_count[i] != 0 && have_zero_gyro_error_count) {
                ...
                _gyro_healthy[i] = false;
                hal.console->println_P(PSTR("Gyro not healthy!!!"));
            }
        ...
        for (uint8_t i=0; i<INS_MAX_INSTANCES; i++) {
            if (_gyro_healthy[i]) {
				//hal.console->println_P(PSTR("Has gyro!!!"));
                _primary_gyro = i;
                break;
            }
        }
        for (uint8_t i=0; i<INS_MAX_INSTANCES; i++) {
            if (_accel_healthy[i]) {
                _primary_accel = i;
                //hal.console->println_P(PSTR("Has accel!!!"));
                break;
            }
        }
    }
...
check_sample:
    if (!_hil_mode) {
		hal.console->print_P(PSTR("Entered check sample\n"));
        ...
        hal.console->print_P(PSTR("Exit check sample\n"));
    }
