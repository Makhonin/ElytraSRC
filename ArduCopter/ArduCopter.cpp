...
void Copter::setup() 
{
    ...
    cliSerial->print_P(PSTR("Ready for fast loop! OK\n"));
}
...
void Copter::loop()
{
	//cliSerial->print_P(PSTR("Loop1\n"));
    // wait for an INS sample
    ins.wait_for_sample();
	//cliSerial->print_P(PSTR("Loop2\n"));
    uint32_t timer = micros();
	//cliSerial->print_P(PSTR("Loop3\n"));
    // check loop time
    perf_info_check_loop_time(timer - fast_loopTimer);
	//cliSerial->print_P(PSTR("Loop4\n"));
    // used by PI Loops
    ...
    // ---------------------
    fast_loop();
	//cliSerial->print_P(PSTR("Loop5\n"));
    // tell the scheduler one tick has passed
    scheduler.tick();
	//cliSerial->print_P(PSTR("Loop6\n"));
   ...
   // cliSerial->print_P(PSTR("Loop7\n"));
    scheduler.run(time_available);
    //cliSerial->print_P(PSTR("Loop8\n"));
}


// Main loop - 400hz !!!!!!!!!!!!!!! not 100
void Copter::fast_loop()
{
	...
	motors.set_conv(attitude_control.get_conversion_function());
